...
export async function GET(request: NextRequest) {
  ...
  if (!META_APP_ID || !REDIRECT_URI) {
    return NextResponse.json(
      { error: "WhatsApp integration is not configured on this server. Add META_APP_ID and META_WHATSAPP_REDIRECT_URI to your environment variables." },
      { status: 503 }
    )
  }

  // Nonce ties the cookie to the state param — prevents CSRF
